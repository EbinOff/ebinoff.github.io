...
async function openGalleryExportDialog() {
  ...
  // Show dialog immediately, but disable export until estimate is ready
  if (galleryExportDialog) galleryExportDialog.style.display = "flex";
  setGalleryExportBtnLoading(true);

  await updateGalleryExportInfo(thumbs, selectionType, selectedGalleryExportFormat);
  setGalleryExportBtnLoading(false);
}

function setGalleryExportBtnLoading(isLoading) {
  const btn = document.getElementById("galleryExportBtnDoExport");
  if (!btn) return;
  if (isLoading) {
    btn.disabled = true;
...
function renderGalleryExportFormats(formats) {
  ...
  formats.forEach((format) => {
    ...
    button.addEventListener("click", async () => {
      ...
      renderGalleryExportFormats(formats);
      const thumbs = getSelectedThumbsInGalleryOrder();
      await updateGalleryExportInfoWithLoadingState(thumbs, classifyGallerySelection(thumbs), format);
    });
...
// Also update format-switching to show loading again while re-estimating
async function updateGalleryExportInfoWithLoadingState(thumbs, selectionType, format) {
  setGalleryExportBtnLoading(true);
  await updateGalleryExportInfo(thumbs, selectionType, format);
  setGalleryExportBtnLoading(false);
}
...
async function exportGalleryImagesAsZip(thumbs, format) {
  ...
  for (let i = 0; i < thumbs.length; i++) {
    ...
    if (thumb.dataset.mimeType === mimeType) {
      // Use highResUrl to get the full-quality original for ZIP exports
      const blob = await fetchBlobFromUrl(thumb.dataset.highResUrl || thumb.dataset.url);
...
async function exportGalleryImages(thumbs, format) {
  ...
  let blob;
  if (thumb.dataset.mimeType === mimeType) {
    blob = await fetchBlobFromUrl(thumb.dataset.url);
  } else {
    blob = await getThumbBlobForExport(thumb, mimeType, quality);
  }
...
async function estimateThumbExportBytes(thumb, mimeType = "image/png", quality = 1) {
  // Use highResUrl for accurate file size estimation
  const srcUrl = thumb.dataset.highResUrl || thumb.dataset.url;
  if (thumb.dataset.mimeType === mimeType) {
    if (thumb.dataset.fileSize) return parseInt(thumb.dataset.fileSize);
    const blob = await fetchBlobFromUrl(srcUrl);
    return blob.size;
  }

  const image = await loadImageElement(thumb.dataset.highResUrl || thumb.dataset.url);
...
async function getThumbCanvasForExport(thumb, mimeType = "image/png", _unused) {
  // Use highResUrl for export to get full-quality output
  const srcUrl = thumb.dataset.highResUrl || thumb.dataset.url;
...
async function overwriteImageThumbFromBlob(thumb, blob) {
  ...
  const oldHighResUrl = thumb.dataset.highResUrl;
  const newUrl = URL.createObjectURL(blob);

  // Store the new full-res URL as highResUrl, and the low-res thumb as url
  thumb.dataset.highResUrl = newUrl;
  thumb.dataset.url = newUrl; // fallback still works if highResUrl is missing elsewhere
  ...
  if (oldUrl && oldUrl !== oldHighResUrl) URL.revokeObjectURL(oldUrl);
  if (oldHighResUrl) URL.revokeObjectURL(oldHighResUrl);
}
...
function setupThumb(thumb, fileURL, isImage, pdfDoc = null) {
  ...
  editBtn.onclick = async (e) => {
    ...
    // 3. STANDARD IMAGE (Safety Net)
      ...
      // Use highResUrl if available (progressive loading), else fall back to url
      const srcToOpen = thumb.dataset.highResUrl || thumb.dataset.url;
      const tempImg = new Image();
      tempImg.onload = () => {
        openImageModal(srcToOpen, false, thumb, { onSave: handleSave });
      };
      tempImg.onerror = () => {
        alert("Image memory was cleared by the browser. Please re-import the file.");
      };
      tempImg.src = srcToOpen;
    }
  };
...
function addFile(file) {
  ...
  if (isImage) {
    ...
    thumb.dataset.sourceName = uniqueName;
    thumb.dataset.copyCount = "0";
    thumb.dataset.highResUrl = fileURL; // Store full-res separately

    addFormatBadge(thumb, uniqueName);
    setupThumb(thumb, fileURL, true);

    // Progressive loading: show a placeholder immediately, then generate low-res thumb
    img.src = fileURL; // show full-res initially for speed; swap after thumbnail is ready
    createThumbPreviewFromBlob(file).then(lowResDataUrl => {
      img.src = lowResDataUrl; // swap to low-res for gallery display
    }).catch(() => { /* keep original if thumb generation fails */ });
...
window.clearDragClasses = function clearDragClasses() {
...
document.addEventListener("dragleave", (e) => {
  dragCounter--;
  if (dragCounter === 0) window.clearDragClasses();
});
...
document.addEventListener("DOMContentLoaded", refreshAdSense);
