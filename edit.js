                    ...
                    <div class="export-section">
                        ...
                        <div class="export-page-options">
                            ...
                            <label class="export-radio">
                                ...
                                <span id="selectedPagesLabel">Selected Pages</span>
                            </label>
                        </div>
                    </div>
...
async function getLiveToolPreviewEstimatedBytes() {
    ...
    if (isResizeToolVisible()) {
        ...
        // Return both PNG and JPG estimates for resize
        const pngBytes = await estimateCanvasBytes(canvas, "image/png", 1);
        const jpgBytes = await estimateCanvasBytes(canvas, "image/jpeg", 0.85);
        return { pngBytes, jpgBytes };
    }
    ...
    if (isCompressToolVisible()) {
        ...
        const jpgBytes = await estimateCanvasBytes(canvas, "image/jpeg", quality);
        return { jpgBytes };
    }
...
async function updateToolInfo() {
    ...
    toolInfoBar.textContent = "Est. Size: calculating...";

    try {
        const result = await getLiveToolPreviewEstimatedBytes();
        if (token !== toolInfoUpdateToken || !toolInfoBar) return;

        if (isResizeToolVisible() && result.pngBytes !== undefined) {
            // Show both PNG and JPG estimates for resize
            toolInfoBar.textContent = `Est. Size: png- ${formatBytes(result.pngBytes)} jpg- ${formatBytes(result.jpgBytes)}`;
        } else {
            // Show only JPG estimate for compress (PNG is unaffected by JPEG compression)
            toolInfoBar.textContent = `Est. Size: ${formatBytes(result.jpgBytes)}(jpg)`;
        }
    } catch {
        if (token !== toolInfoUpdateToken || !toolInfoBar) return;
        toolInfoBar.textContent = "Est. Size: unavailable";
    }
}
...
window.setupToolZoom = function () {
    ...
    const applyToolViewportRules = (wrapper) => {
        ...
        if (currentZoom > MIN_ZOOM) {
            // Keep padding and layout intact so flexbox doesn't cause the image to jump
...
function setEditorButtonsDisabled(disabled) {
    ...
    if (!modal) return;
    // Disable all modal-btn, sidebar-btn, etc. but NOT the close button
    const allBtns = modal.querySelectorAll(
        '.modal-btn, .undo-btn, .sidebar-footer button, #exportBtnMain, #btnZoomIn, #btnZoomOut, #btnRotateCCW, #btnRotateCW, #btnFlipH, #btnFlipV, #btnCrop, #btnResize, #btnCompress'
    );
    allBtns.forEach(btn => { btn.disabled = disabled; });
}

// Shows/hides the full-screen loading overlay on the active carousel page
function setCarouselLoadingOverlay(show, wrapper) {
    if (!wrapper) return;
    let overlay = wrapper.querySelector('.carousel-loading-overlay');
    if (show) {
        if (!overlay) {
            overlay = document.createElement('div');
            overlay.className = 'carousel-loading-overlay';
            overlay.innerHTML = `<div class="carousel-loading-spinner"></div><span>Loading...</span>`;
            wrapper.appendChild(overlay);
        }
...
function renderImageContent(src, originalThumb) {
    totalPages = 1;
    slider.max = 0;
    addPageSkeleton(0, false);
    // Use highResUrl for the editor if available, else use src directly
    const highResSrc = (originalThumb && originalThumb.dataset.highResUrl) || src;
    fillPageContent(highResSrc, 0, false, src !== highResSrc);
}
...
function fillPageContent(src, index, isPdf, showLoadingOverlay = false) {
    ...
    if (showLoadingOverlay) {
        // Show loading overlay, disable toolbar, then load high-res
        setCarouselLoadingOverlay(true, wrapper);
        setEditorButtonsDisabled(true);

        img.onload = () => {
            setCarouselLoadingOverlay(false, wrapper);
...
function setupExportHandler() {
    ...
    const selectedPagesLabel = document.getElementById("selectedPagesLabel");
    ...
    function setExportDialogLoading(isLoading) {
        if (!btnExport) return;
        btnExport.disabled = isLoading;
        btnExport.textContent = isLoading ? "Loading..." : "Export";
    }

    function updateSelectedPagesLabel() {
        if (!selectedPagesLabel) return;
        selectedPagesLabel.textContent = selectedPages.size > 0
    ...
    btnExport.onclick = async () => {
        ...
        // Hide entire page range section if only 1 page (no choice needed)
        const pageRangeSection = document.querySelector(".export-page-options");
        const pageRangeLabel = pageRangeSection ? pageRangeSection.previousElementSibling : null;
        const pageRangeSectionParent = pageRangeSection ? pageRangeSection.closest(".export-section") : null;
        const hidePageOptions = totalPages <= 1 || (currentEditorMode === "grid" && selectedPages.size === 1);
        if (hidePageOptions) {
            if (pageRangeSectionParent) pageRangeSectionParent.style.display = "none";

            if (currentEditorMode === "grid" && selectedPages.size === 1) {
                selectedPageRange = "selected";
                const selectedRadio = Array.from(pageRangeRadios).find(radio => radio.value === "selected");
            ...
            if (pageRangeSectionParent) pageRangeSectionParent.style.display = "";
            ...
            if (selectedPages.size === 0) {
                ...
                if (currentEditorMode === "grid" && currentPageLabel) {
    ...
    // Close dialog
    exportDialogClose.onclick = () => {
        exportDialog.style.display = "none";
        setExportDialogLoading(false);
    };
    exportBtnCancel.onclick = () => {
        exportDialog.style.display = "none";
        setExportDialogLoading(false);
    };
    ...
    async function estimateExportSize(pageIndices, format) {
        ...
        let totalBytes = 0;
        for (const pageIdx of pageIndices) {
            const formatMimeType = format === "png" ? "image/png" : "image/jpeg";
            const wrapper = carousel.children[pageIdx];
            const img = wrapper?.querySelector("img, canvas");

            const history = pageHistory[pageIdx] || [];
            const ptr = historyPointers[pageIdx] || 0;
            const currentState = history[ptr]?.state || { rotation: 0, flipH: 1, flipV: 1 };
            const isUnmodified = !isDirty && currentState.rotation === 0 && currentState.flipH === 1 && currentState.flipV === 1;

            const currentSrc = img && img.src ? img.src : "";

            if (isUnmodified && currentSrc.startsWith(`data:${formatMimeType};`)) {
                const base64Len = currentSrc.split(",")[1].length;
                totalBytes += Math.floor(base64Len * 0.75);
            } else if (isUnmodified && currentSrc.startsWith("blob:") && originalThumb && originalThumb.dataset.mimeType === formatMimeType && originalThumb.dataset.fileSize) {
                totalBytes += parseInt(originalThumb.dataset.fileSize);
            } else {
                const canvas = await getBakedPageCanvas(pageIdx);
                const quality = format === "png" ? 1 : 0.85; // Default to 0.85 to match main.js
                totalBytes += await estimateCanvasBytes(canvas, formatMimeType, quality);
            }
        }
    ...
    async function exportAsImageOutput(pageIndices, baseName, format) {
        if (pageIndices.length === 1) {
            ...
            let blob;
            if (isUnmodified && currentSrc.startsWith(`data:${mimeType};`)) {
                blob = await fetch(currentSrc).then(r => r.blob());
            } else if (isUnmodified && currentSrc.startsWith("blob:") && originalThumb && originalThumb.dataset.mimeType === mimeType) {
                blob = await fetch(currentSrc).then(r => r.blob());
            } else {
                const canvas = await getBakedPageCanvas(pageIdx);
                const quality = format === "png" ? 1 : 0.85;
                blob = await canvasToBlobPromise(canvas, mimeType, quality);
            }
        ...
        for (let i = 0; i < pageIndices.length; i++) {
            ...
            let base64;
            if (isUnmodified && currentSrc.startsWith(`data:${mimeType};`)) {
                base64 = currentSrc.split(",")[1];
            } else if (isUnmodified && currentSrc.startsWith("blob:") && originalThumb && originalThumb.dataset.mimeType === mimeType) {
                const blob = await fetch(currentSrc).then(r => r.blob());
                const reader = new FileReader();
                await new Promise(resolve => {
                    reader.onload = () => { base64 = reader.result.split(",")[1]; resolve(); };
...
window.setupResizeTool = () => {
    ...
    let resizePreviewToken = 0;

    const applyResizePreview = async () => {
        if (!resizeW || !resizeH) return;
        const token = ++resizePreviewToken;
        const activeWrapper = carousel.children[activeIndex];
        ...
        if (token !== resizePreviewToken) return;

        let w = parseInt(resizeW.value);
        let h = parseInt(resizeH.value);
        if (isNaN(w) || w < 1) w = 1;
        if (isNaN(h) || h < 1) h = 1;
        ...
        previewImg.onload = async () => {
            ...
            if (img.tagName.toUpperCase() === "IMG") {
                img.onload = () => {
                    // Don't call scrollToPage here — it causes visual jitter by
                    // re-scrolling the carousel. Just refresh the zoom viewport.
                    if (typeof window.refreshToolZoomViewport === "function") window.refreshToolZoomViewport();
                    updateToolInfo();
                };
                img.src = previewUrl;
    ...
    if (resizeScale) {
        ...
        resizeW.oninput = (e) => {
            let newW = parseInt(e.target.value);
            if (isNaN(newW) || newW < 1) { newW = 1; resizeW.value = 1; }
            if (isRatioLocked) {
                const ratio = origImgH / origImgW;
                resizeH.value = Math.max(1, Math.round(newW * ratio));
        ...
        resizeH.oninput = (e) => {
            ...
            if (isNaN(newH) || newH < 1) { newH = 1; resizeH.value = 1; }
            if (isRatioLocked) {
                const ratio = origImgW / origImgH;
                resizeW.value = Math.max(1, Math.round(newH * ratio));
                        ...
                        if (img && img.dataset.pristine) {
                            await new Promise((resolve) => {
                                if (img.tagName.toUpperCase() === 'IMG') {
                                    ...
                                    pristineImg.onload = () => {
                                        img.width = pristineImg.width;
                                        img.height = pristineImg.height;
                                        const ctx = img.getContext("2d");
                                        ctx.clearRect(0, 0, img.width, img.height);
                                        ctx.drawImage(pristineImg, 0, 0);
                                        resolve();
                                    };
                                    pristineImg.src = img.dataset.pristine;
                                }
                            });
        ...
        if (btnConfirmResize) {
            ...
            btnConfirmResize.onclick = async () => {
                let finalW = parseInt(resizeW.value);
                let finalH = parseInt(resizeH.value);
                if (isNaN(finalW) || finalW < 1) finalW = 1;
                if (isNaN(finalH) || finalH < 1) finalH = 1;
                ...
                const img = activeWrapper.querySelector("img, canvas");
                const sourceSrc = img.dataset.pristine || (img.tagName.toUpperCase() === "CANVAS" ? img.toDataURL("image/png") : img.src);
                const sourceImg = new Image();
                await new Promise(resolve => {
                    sourceImg.onload = resolve;
...
window.setupCompressTool = () => {
    ...
    if (btnCompressTrigger) {
        btnCompressTrigger.onclick = () => {
            ...
            if (typeof window.setupToolZoom === 'function') window.setupToolZoom();
            if (typeof window.toggleToolZoomButtons === 'function') window.toggleToolZoomButtons(true);
            // Removed hardcoded reset to 80 so it remembers previous state like resize tool
            suppressPresetReset = false;
            // Sync the label to match whatever the current slider value is
            if (compressQualityValue && compressQuality) compressQualityValue.textContent = `${compressQuality.value}%`;
            ...
            const applyPaddingWhenReady = () => {
                ...
                // Apply compress preview immediately (generates the live estimated size)
                applyCompressPreview();
            };
